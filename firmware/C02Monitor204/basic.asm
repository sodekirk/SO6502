;
; Enhanced BASIC for the C02 Pocket SBC, Version 2.22p5C02
...
; "C02" is appemded to define CMOS only and working with C02Monitor/C02BIOS
...
;
; NOTE: a couple patches to correct p5 patches on 27th February 2020 - oops
;
; NOTE: startup code is now contained here. Any Monitor or OS can now call the
; ROM start location of EhBasic. Five Monitor/OS routines are required for the
; core functions required: CHRIN, CHROUT, LOAD, SAVE, EXIT.
; These are defined below. - 31st March, 2020
;
; Several of the Basic messages have been abbreviated to keep ROM space below $2700 bytes.
; Well, We blew past $2700 on size after including all the startup code, so Basic messages
; have been restored to their original text... sniff. It's still less than 10KB, so yeah!
; Latest version shows as version 2.22p5C02 - 23rd May, 2020
;
...
;  Note: new instructions used include SMBx, RMBx, BBSx, BBRx! Note that
;  this version will NOT run on a W65C816 in emulation mode!!
;  Version renamed to Ver 2.22p5C02 (concurrency with Klaus' patched version)
;
...
; - RETNMI
; - OFF
; It is known that the interrupt functions above were not fully implemented
; and didn't really work without additional coding. As the NMI routine is a
; Panic routine on the C02 Pocket SBC, there's no need to support this one.
; Also, the C02 Pocket SBC has a full IRQ code stack that is extendable. It
; provides full support for the SCC2691 UART for Transmit, Receive, Break,
; a 10ms jiffy-clock plus delays and a benchmark timer. These routines can
; be called via JMP table entries in the C02 BIOS and Monitor. They could
; also be added as additional Basic Commands, but I don't have any current
; plan to do so as of now.
;
; NOTE: BASIC Tokens are NOT the same due to removal of the above commands!!
; This would only be a problem if someone did a binary save of a BASIC program
; from the original code and then loaded it to this version of Basic. This is
; unlikely however, as the original Enhanced Basic had NO Load or Save functions
; implemented! Also note that Lee (when asked) recommended that Load and Save be
; implemented using the LIST function redirected to Save and Basic Entry for Load.
;
; NOTE: The LOAD and SAVE functions implemented in this version are done as Binary!
; The actual Load and Save routines are contained in the C02 Monitor code and called
; via vectors. The C02 Monitor uses Xmodem-CRC for transferring data to and from
; the host machine running the terminal emulator. Serial (OSX) and ExtraPutty have
; been tested and work properly.
;
...
;
; 2.22p5C   Updates from Klaus for specific bug fixes added to the CMOS version
;
ZPSTART         .EQU $00        ; Start of zero page workspace
;
LAB_WARM        .EQU ZPSTART    ; $00=BASIC warm start entry point
Wrmjpl          .EQU LAB_WARM+1 ; BASIC warm start vector jump low byte
Wrmjph          .EQU LAB_WARM+2 ; BASIC warm start vector jump high byte

Usrjmp          .EQU Wrmjph+1   ; USR function JMP address
Usrjpl          .EQU Usrjmp+1   ; USR function JMP vector low byte
Usrjph          .EQU Usrjmp+2   ; USR function JMP vector high byte

Nullct          .EQU Usrjph+1   ; nulls output after each line
TPos            .EQU Nullct+1   ; BASIC terminal position byte
TWidth          .EQU TPos+1     ; BASIC terminal width byte
Iclim           .EQU TWidth+1   ; input column limit
Itempl          .EQU Iclim+1    ; temporary integer low byte
...
Srchc           .EQU nums_3+1   ; search character
Temp3           .EQU Srchc      ; temp byte used in number routines
Scnquo          .EQU Srchc+1    ; scan-between-quotes flag
...
Ibptr           .EQU Scnquo+1   ; input buffer pointer
Dimcnt          .EQU Ibptr      ; # of dimensions
Tindx           .EQU Ibptr      ; token index

Defdim          .EQU Ibptr+1    ; default DIM flag
Dtypef          .EQU Defdim+1   ; data type flag, $FF=string, $00=numeric
Oquote          .EQU Dtypef+1   ; open quote flag (b7) (Flag: DATA scan; LIST quote; memory)
Gclctd          .EQU Oquote     ; garbage collected flag
Sufnxf          .EQU Gclctd+1   ; subscript/FNX flag, 1xxx xxx = FN(0xxx xxx)
Imode           .EQU Sufnxf+1   ; input mode flag, $00=INPUT, $80=READ

Cflag           .EQU Imode+1    ; comparison evaluation flag

TabSiz          .EQU Cflag+1    ; TAB step size (was input flag)

next_s          .EQU TabSiz+1   ; next descriptor stack address

; these two bytes form a word pointer to the item
; currently on top of the descriptor stack

last_sl         .EQU next_s+1   ; last descriptor stack address low byte
last_sh         .EQU last_sl+1  ; last descriptor stack address high byte (always $00)

des_sk          .EQU last_sh+1  ; descriptor stack start address (temp strings)

ut1_pl          .EQU des_sk+9   ; utility pointer 1 low byte
ut1_ph          .EQU ut1_pl+1   ; utility pointer 1 high byte
ut2_pl          .EQU ut1_ph+1   ; utility pointer 2 low byte
ut2_ph          .EQU ut2_pl+1   ; utility pointer 2 high byte

Temp_2          .EQU ut1_pl     ; temp byte for block move

FACt_1          .EQU ut2_ph+1   ; FAC temp mantissa1
...
TempB           .EQU FACt_1+3   ; temp page 0 byte

Smeml           .EQU TempB+1    ; start of mem low byte         (Start-of-Basic)
Smemh           .EQU Smeml+1    ; start of mem high byte        (Start-of-Basic)
Svarl           .EQU Smemh+1    ; start of vars low byte        (Start-of-Variables)
Svarh           .EQU Svarl+1    ; start of vars high byte       (Start-of-Variables)
Sarryl          .EQU Svarh+1    ; var mem end low byte          (Start-of-Arrays)
Sarryh          .EQU Sarryl+1   ; var mem end high byte         (Start-of-Arrays)
Earryl          .EQU Sarryh+1   ; array mem end low byte        (End-of-Arrays)
Earryh          .EQU Earryl+1   ; array mem end high byte       (End-of-Arrays)
Sstorl          .EQU Earryh+1   ; string storage low byte       (String storage (moving down))
Sstorh          .EQU Sstorl+1   ; string storage high byte      (String storage (moving down))
Sutill          .EQU Sstorh+1   ; string utility ptr low byte
Sutilh          .EQU Sutill+1   ; string utility ptr high byte
Ememl           .EQU Sutilh+1   ; end of mem low byte           (Limit-of-memory)
Ememh           .EQU Ememl+1    ; end of mem high byte          (Limit-of-memory)
Clinel          .EQU Ememh+1    ; current line low byte         (Basic line number)
Clineh          .EQU Clinel+1   ; current line high byte        (Basic line number)
Blinel          .EQU Clineh+1   ; break line low byte           (Previous Basic line number)
Blineh          .EQU Blinel+1   ; break line high byte          (Previous Basic line number)

Cpntrl          .EQU Blineh+1   ; continue pointer low byte
Cpntrh          .EQU Cpntrl+1   ; continue pointer high byte

Dlinel          .EQU Cpntrh+1   ; current DATA line low byte
Dlineh          .EQU Dlinel+1   ; current DATA line high byte

Dptrl           .EQU Dlineh+1   ; DATA pointer low byte
Dptrh           .EQU Dptrl+1    ; DATA pointer high byte

Rdptrl          .EQU Dptrh+1    ; read pointer low byte
Rdptrh          .EQU Rdptrl+1   ; read pointer high byte

Varnm1          .EQU Rdptrh+1   ; current var name 1st byte
Varnm2          .EQU Varnm1+1   ; current var name 2nd byte

Cvaral          .EQU Varnm2+1   ; current var address low byte
Cvarah          .EQU Cvaral+1   ; current var address high byte

Frnxtl          .EQU Cvarah+1   ; var pointer for FOR/NEXT low byte
...
prstk           .EQU Frnxtl+2   ; precedence stacked flag

comp_f          .EQU prstk+2    ; compare function flag, bits 0,1 and 2 used
                                ; bit 2 set if >
                                ; bit 1 set if =
                                ; bit 0 set if <

func_l          .EQU comp_f+1   ; function pointer low byte
...
des_2l          .EQU func_h+1   ; string descriptor_2 pointer low byte
des_2h          .EQU des_2l+1   ; string descriptor_2 pointer high byte

g_step          .EQU des_2l+2   ; garbage collect step size

Fnxjmp          .EQU g_step+1   ; jump vector for functions
...
FAC2_r          .EQU Fnxjmp+2   ; FAC2 rounding byte

Adatal          .EQU FAC2_r+1   ; array data pointer low byte
...
Obendl          .EQU Adatah+1   ; old block end pointer low byte
Obendh          .EQU Obendl+1   ; old block end pointer high byte

numexp          .EQU Obendh+1   ; string to float number exponent count
expcnt          .EQU numexp+1   ; string to float exponent count

numbit          .EQU numexp     ; bit count for array element calculations

numdpf          .EQU expcnt+1   ; string to float decimal point flag
expneg          .EQU numdpf+1   ; string to float eval exponent -ve flag
...
FAC1_e          .EQU expneg+1   ; FAC1 exponent
...
negnum          .EQU FAC1_e+5   ; string to float eval -ve flag
numcon          .EQU negnum     ; series evaluation constant count

FAC1_o          .EQU negnum+1   ; FAC1 overflow byte

FAC2_e          .EQU FAC1_o+1   ; FAC2 exponent
...
FAC_sc          .EQU FAC2_e+5   ; FAC sign comparison, Acc#1 vs #2
FAC1_r          .EQU FAC_sc+1   ; FAC1 rounding byte
...
csidx           .EQU FAC1_r+1   ; line crunch save index
Asptl           .EQU csidx      ; array size/pointer low byte
Aspth           .EQU csidx+1    ; array size/pointer high byte
...
;
Bpntrl          .EQU Sendh+1    ; BASIC execute (get byte) pointer low byte
Bpntrh          .EQU Bpntrl+1   ; BASIC execute (get byte) pointer high byte

Rbyte4          .EQU Bpntrh+1   ; extra PRNG byte
...
Decss           .EQU Rbyte3+1   ; number to decimal string start
Decssp1         .EQU Decss+1    ; number to decimal string start
ZPLastByte      .EQU Decss+17   ; last declared byte in Page Zero
...
TK_DOKE           .EQU TK_POKE+1       ; DOKE token
TK_CALL           .EQU TK_DOKE+1       ; CALL token
...
TK_EXIT           .EQU TK_BITCLR+1     ; EXIT token

; secondary command tokens, can't start a statement

TK_TAB            .EQU TK_EXIT+1       ; TAB token
...
TK_DEEK           .EQU TK_PEEK+1       ; DEEK token
TK_SADD           .EQU TK_DEEK+1       ; SADD token
...
TK_TWOPI          .EQU TK_PI+1         ; TWOPI token
TK_VPTR           .EQU TK_TWOPI+1      ; VARPTR token
...
STACK_RES         .EQU #20       ; reserved stack space

LAB_STAK          .EQU $0100     ; stack bottom, no offset

LAB_SKFE          .EQU LAB_STAK+$FE
                                 ; flushed stack address
LAB_SKFF          .EQU LAB_STAK+$FF
                                 ; flushed stack address
...
Ibuffs          .EQU $0400     ; Start of input buffer
Ibuffe          .EQU $0400+$7E ; end of input buffer (127 bytes)
...
LAB_FOR
      SMB7  Sufnxf            ; set Subscript/FNx flag
;      LDA   #$80              ; set FNX
...
LAB_1904
      BBR7  Imode,LAB_1913    ; Check input mode, branch if INPUT ($00)
;      LDA   Imode             ; get input mode flag, $00=INPUT, $80=READ
...
LAB_195B
      ...
      BBS7  Imode,LAB_19DD    ; test Input mode, branch if $80=READ
...
LAB_1A03
      ...
      BBR7  Imode,LAB_1A0E    ; test input mode, branch if $00=INPUT
...
;
      .word B_CHRIN_NW        ; byte in from C02BIOS (no waiting)
      .word B_CHROUT          ; byte out from C02BIOS
      .word M_LOAD            ; Xmodem LOAD for C02 Monitor
      .word M_SAVE            ; Xmodem SAVE for C02 Monitor
      .word M_EXIT            ; Warm start for C02 Monitor
...
BOOT_BASIC
; To conserve some space in the C02Monitor allocated EEPROM space, the startup
; routine has been moved here. The goal is to ensure that EnHanced Basic is
; fully self contained and requires nothing but the ROM vectors to function.
; This adds some additional coding, but still ensures total allocated space
; is within 10KB (10240 bytes) of EEPROM space.

      LDA   #<LAB_CSMSG          ; point to init message (low addr)
      LDY   #>LAB_CSMSG          ; point to init message (high addr)
      STA   PROMPTL              ; Store low byte
      STY   PROMPTH              ; Store high byte
PROMPT2
      LDA   (PROMPTL)            ; Get string data
      BEQ   EHB_TRY2             ; If null character, branch to user entry
...
EHB_TRY2
      JSR   B_CHRIN_NW           ; Request keystroke input from terminal
      BCC   EHB_TRY2             ; If carry clear, nothing entered, loop back
...
UCOK
      ...
      LDA   #$07                 ; Get bell character
      JSR   B_CHROUT             ; Else, beep for entry error
      BRA   EHB_TRY2             ; Branch back and try again

EHB_WARM
      JMP   $0000                ; Warm start soft vector

EHB_COLD
      JMP   LAB_COLD             ; Cold start routine

LAB_CSMSG
      .byte $0D,$0A
      .byte "65C02 Enhanced BASIC Version 2.22p5C02"
      .byte $0D,$0A
      .byte " [C]old/[W]arm start? ",$00
