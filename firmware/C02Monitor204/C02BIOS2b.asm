;**************************************************************************************************
;*  C02BIOS 2.04(c)2013-2019 by Kevin E. Maier    *     New Hardware design with the following:   *
...
;
;**************************************************************************************************
IOPAGE         .EQU     $8000            ;I/O Page Base Start Address
;**************************************************************************************************
...
SCCTPL  .EQU SCBase+7
SCIPR   .EQU SCBase+13
SCOPCR  .EQU SCBase+13
SCSTACT .EQU SCBase+14
SCSTPCT .EQU SCBase+15
...
;                               START OF BIOS CODE                                                *
;**************************************************************************************************
;C02BIOS version used here is 2.04 (updated release)
...
; - Input/Code from Michael Barry - modified RTC routine - saves 15 bytes
...
;
; $FF30 CNT_STRT        ;Reset/Start 10ms benchmark timing counter
...
;
               STZ      SCIMR           ;Disable ALL interrupts from UART
               LDA      SCSRA           ;Get UART Status Register
               STA      UART_SRT        ;Save it in Page 0
               LDA      SCISR           ;Get UART Interrupt Status Register
...
;**************************************************************************************************
;
UART_RCV       LDY      ICNT            ;Get buffer counter (3)
               BMI      BUFFUL          ;Check against limit, branch if full (2/3)
               LDA      SCRXA           ;Else, get character from 2691 (4)
;
...
;   enables an interrupt whenever a character is received, so the FIFO will not fill up unless
...
;
INTERUPT0      LDA      SCISR           ;Get the UART Interrupt Status Register (4)
               ;CMP      #%00100000      ;Check for no active IRQ source (2)
               BEQ      REGEXT0         ;If no bits are set, exit handler (2/3)
;
               BIT      #%00000100      ;Test for Delta Break (2)
               BNE      UART_BRK        ;If yes, Reset the UART receiver (2/3)
               BIT      #%00000010      ;Test for RHR having a character (2)
               BNE      UART_RCV        ;If yes, put the character in the buffer (2/3)
               BIT      #%00000001      ;Test for THR ready to receive a character (2)
               BNE      UART_XMT        ;If yes, get character from buffer (2/3)
...
IRQEXT0        STA      UART_IRT        ;Else, save the 2691 IRT for later use (4)
               LDA      SCSRA           ;Get 2691 Status Register (4)
...
;
UART_RTC       ;LDA      #%11110000      ;Get Command mask for stop timer (2)
               ;STA      SCCRA           ;Send command to 2691 (4)
               LDA      SCSTPCT         ;Send command to 2691 (4)
;
...
;
               ...
               BNE      REGEXT0         ;If not zero, exit (2/3)
...
; Note that Mike's code is a longer execution time, albeit mostly not
; significant. If allocated BIOS space gets tight, this provides some relief.
;
...
;               BRA      REGEXT0         ; Then exit IRQ handler (3)
;
UART_BRK       LDA      SCSRA           ;Get UART Status Register (4)
...
BREAKEY        LDA      #%01000000      ;Get Reset Received Break command (2)
               STA      SCCRA           ;Send to UART to reset (4)
...
;
INIT_IO        JSR      RESET_2892      ;Power-Up Reset of SCC2691 UART
...
;
B_Reserve00    JMP      RESERVE         ;Call 00 $FF00
B_Reserve01    JMP      RESERVE         ;Call 01 $FF03
B_Reserve02    JMP      RESERVE         ;Call 02 $FF06
B_Reserve03    JMP      RESERVE         ;Call 03 $FF09
B_Reserve04    JMP      RESERVE         ;Call 04 $FF0C
B_Reserve05    JMP      RESERVE         ;Call 05 $FF0F
B_Reserve06    JMP      RESERVE         ;Call 06 $FF12
B_Reserve07    JMP      RESERVE         ;Call 07 $FF15
B_Reserve08    JMP      RESERVE         ;Call 08 $FF18
B_Reserve09    JMP      RESERVE         ;Call 09 $FF1B
B_Reserve10    JMP      RESERVE         ;Call 10 $FF1E
...
B_Reserve15    JMP      RESERVE         ;Call 15 $FF2D
;
...
B_INIT_2892    JMP      INIT_2892       ;Call 27 $FF51
B_RESET_2892   JMP      RESET_2892      ;Call 28 $FF54
;
...
;
;Data for RTC interrupt service routine (ISR)
; Table data to test for maximum counts: Seconds, Minutes, Hours, Days (L/H)
; see code above in RTC interrupt section for using this as an alternate.
;
;SMHDLIM       .DB     59,59,23,255,255  ;RTC table values
;
...
;
        .ORG    $FFC0   ;Hard code BIOS message to the top of memory (Monitor uses this)
...
BIOS_MSG       .DB      $0D,$0A         ;CR/LF
               .DB      "C02BIOS 2.04"  ;Updated Release Version
               .DB      $0D,$0A         ;CR/LF
               .DB      "30/09/2019"    ;DD/MM/YYYY
               .DB      $0D,$0A         ;CR/LF
               .DB      "W65C02@"       ;Display CPU type
CPU_CLK        .DB      "10MHz"          ;Displayed CPU Clock frequency
