;**************************************************************************************************
;*   C02Monitor 2.04- Release version for Pocket SBC  (c)2013-2019 by Kevin E. Maier 30/09/2019   *
;*                                                                                                *
...
;*      - CTRL-B: Starts CMOS version of Enhanced Basic Version 2.22p4C                           *
...
;       DATABUFF is used by the HEX2ASC routine (6 bytes). 16-bit to ASCII decimal
...
;
INBUFF          .EQU    BUFF_PG0+00     ;Input Buffer - 4 bytes ($B0-$B3)
DATABUFF        .EQU    BUFF_PG0+04     ;Data Buffer - 6 bytes ($B4-$B9)
;
;       - 16-bit variables:
...
CMDFLAG         .EQU    PGZERO_ST+37    ;Command Flag - bit specific, used by multiple routines
...
MSDELAY        .EQU     BIOS_PG0+21     ;Timer delay countdown byte (0=256)
...
MATCH          .EQU     BIOS_PG0+29     ;Bit7 used for Delay, Bit6 used for Benchmark
;
;       - Spare BIOS bytes for future use
SPARE_B0       .EQU     BIOS_PG0+30     ;Spare BIOS page zero byte
SPARE_B1       .EQU     BIOS_PG0+31     ;Spare BIOS page zero byte
;
;**************************************************************************************************
...
IRQVEC0         .EQU    SOFTVEC+4       ;IRQ Interrupt Vector 0
;
...
SRCHBUFF        .EQU    $340            ;Located in Page $03 following Hardware config data
;
...
SRBUFF          .EQU    $0350           ;Start of S-Record buffer, up to 44 bytes in length
;
...
; Note: location moved from $00 to $90 to avoid conflict with EhBasic Page 0 usage
;
BURN_BYTE       .EQU    $0090           ;Location in Page 0 RAM for EEPROM BYTE write routine
;**************************************************************************************************
;The following 32 functions are provided by BIOS via the JMP Table below
; $FF00 - $FF2D are Reserved for future expansion
;
...
; $FF39 CHRIN             ;Character input from console, carry set if character
; $FF3C CHROUT            ;Character output to console, sent character preserved
...
M_RESERVE3      JMP     RESERVED        ;Call 03 $E009
M_RESERVE4      JMP     RESERVED        ;Call 04 $E00C
M_RESERVE5      JMP     RESERVED        ;Call 05 $E00F
;
...
M_UPTIME        JMP     UPTIME          ;Call 10 $E01E
...
; Core routine based on Michael Barry's code. Saves many bytes ;-)
...
; set flag after ADC instruction.
                ...
                ADC     TEMP1           ;Add in buffer offset (point to active data only)
                LDY     #>DATABUFF      ;Get High byte address, drop into PROMPTR and finish
;
;PROMPTR routine: takes message address in Y/A and prints via PROMPT routine
...
; xxxxx.xx seconds.
BENCH           LDA     #$2F            ;Get message for Bench startup
                JSR     PROMPT          ;Send message to console
                JSR     CONTINUE2       ;Prompt user (y/n)
                JMP     B_CNT_STRT      ;Call BIOS routine to zero and start count, return
;
...
QUITB           JSR     B_CNT_STOP      ;Stop Benchmark counter
                ...
                LDA     10MS_CNT        ;Get hundreds of seconds
                JSR     HEX8ASC         ;Print hundreds of seconds
                LDA     #$1E            ;Get message for " Seconds"
                JSR     PROMPT          ;Send to console
...
NO_RAM          LDA     #$32            ;Get error message
                JSR     PROMPT          ;Send to console
...
ASSEMBLER       LDA     #$33            ;Get sorry message
                JMP     PROMPT          ;Send to console and return
;
;[CTRL-B] Start Enhanced Basic:
EHBASIC         JMP     $B000           ;Get EhBasic intro Msg
;EHBASIC         LDA     #$35            ;Get EhBasic intro Msg
;                JSR     PROMPT          ;Send to terminal
;;
...
DISASSEMBLER    LDA     #$2E            ;Intro Message
                JSR     PROMPT          ;Send to terminal
                LDA     #$03            ;Msg 03 -" addr:"
...
;
                ...
                LDA     #$25            ;Else, get error msg for failed write
                BRA     PRMPTSC         ;Send to console and exit
;
NOT_ROM         LDA     #$31            ;Get EEPROM address range msg
                BRA     TRY_AGAIN       ;Branch back and try again
;
;[CNTRL-Q] Query command:
QUERY           LDA     #$34            ;Get Query Commands Message
...
DO16TIME        PHX                     ;Push message number to stack
                ...
                BRA     PRMPTSC         ;Send to terminal and return
;
...
XDONE           LDA     #ACK            ;Last block, get ACK character
                ...
                LDA     #$2B            ;Get Good xfer message number
...
STRT_ERR        LDA     #CAN            ;Else get Cancel code
                ...
                LDA     #$2C            ;Get message for receive error
...
S19_LOOP2       LDX     IDX             ;Load current offset to RBUFF
                ...
                CPY     #$00            ;Check if RBUFF to SRBUFF is incomplete
...
XMODEMS         STZ     OPXMDM          ;Clear Xmodem flag
                ...
                STA     BLKNO           ;Save it
;
                LDA     #$29            ;Get Xmodem Save intro msg
...
; Block count has been set to one and the OPXMDM flag has been cleared.
                LDA     #$2A            ;Get Terminal Setup message
                JSR     PROMPT          ;Send to console
;
...
;
                LDA     #$2B            ;Else, get complete msg
XMDM_NE         JMP     PROMPT          ;Send to console and return
;
XMDM_ERR        LDA     #$2C            ;Get xfer fail msg
...
MONCMD  .DB     $01             ;[CNTRL-A] Start Assembler
        .DB     $02             ;[CNTRL-B] Start Enhanced Basic
...
MONTAB  .DW     ASSEMBLER       ;[CNTRL-A] $01 Start Assembler
        .DW     EHBASIC         ;[CNTRL-B] $02 Start Enhanced Basic
        ...
        .DW     UPTIME          ;[CNTRL-T] $14 System uptime from Reset
...
MSG_13  .DB     $0D,$0A
        ...
        .DB     $00
MSG_14  .DB     $0D,$0A
        .DB     "C02Monitor (c)2013-2019 K.E.Maier",$07
        .DB     $0D,$0A
...
MSG_27  .DB     $0D,$0A
        .DB     "Loops: "
        .DB     $00
MSG_28  .DB     "Xmodem Download, <Esc> to abort, or"
        .DB     $0D,$0A
        .DB     "Load Address/S-Record offset:"
        .DB     $00
MSG_29  .DB     "Xmodem Upload, <ESC> to abort, or"
        .DB     $0D,$0A
...
MSG_2E  .DB     $0D,$0A
        .DB     "Disassembly from:"
        .DB     $00
MSG_2F  .DB     "Start Benchmark counter?"
        .DB     $00
MSG_30  .DB     "Edit EEPROM data, be careful!"
        .DB     $00
MSG_31  .DB     $0D,$0A
        .DB     "Address not in EEPROM!"
        .DB     $0D,$0A
        .DB     $00
MSG_32  .DB     $0D,$0A
        .DB     "Address not in RAM!"
        .DB     $0D,$0A
        .DB     $00
MSG_33  .DB     $0D,$0A
        .DB     "Assembler not yet implemented!"
        .DB     $00
MSG_34  .DB     $0D,$0A,$0A
        .DB     "Memory Ops: "
        .DB     "[C]ompare, "
        ...
        .DB     $00
MSG_35  .DB     $0D,$0A
        .DB     "65C02 Enhanced BASIC Version 2.22p4C"
        .DB     $0D,$0A
        .DB     " [C]old/[W]arm start?"
        .DB     $00
MSG_TABLE       ;Message table: contains message addresses sent via the PROMPT routine
        ...
        .DW     MSG_30, MSG_31, MSG_32, MSG_33, MSG_34, MSG_35
;
;**************************************************************************************************
